...
``` 
in file explorer to connect otherwise use
...
const takeScreenshotAndAnalyze = tool({
    ...
    parameters: z.object({
        targetElement: z.string().nullable().optional().describe("What element to look for (e.g., 'Continue button')"),
        analysisType: z.string().nullable().optional().describe("Type of analysis needed"),
    }),
...
const findAndClickElement = tool({
    ...
    async execute({ targetText, elementTypes, waitTime }) {
        ...
        // 1: Try CSS selector with text content using page.evaluateHandle
        try {
        ...
        for (const xpath of xpathSelectors) {
            try {
                ...
                if (elements.length > 0) {
                    await elements[0].evaluate(el => el.scrollIntoView({ behavior: 'auto', block: 'center' }));
                    await delay(500);
